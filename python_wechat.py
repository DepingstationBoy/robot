...
TULING_KEY = '888d508889874a40ab42ddb7dc96ddc4'
xrobot = Tuling(TULING_API, TULING_KEY)

my = itchat.new_instance()
my.auto_login(hotReload=True, enableCmdQR=2)

# my还包括的以下属性，注意用点.查看：
# (1) alive 是否还活着，islogger 是否已登陆
...
def update_my_infos():
    ...
    # set root
    my.to_user_name = [user for user in my.get_friends(update=True) if user['NickName']==u"苦瓜"][0]['UserName']
    return
update_my_infos()
...
class Message(object):
    ...
    def __init__(self, msg):
        ...
        logger.warning("wind_name=%s, send_name=%s, we_type=%s, we_text=%s", self.wind_name, self.actual_remark_name, self.we_type, self.we_text)
        return
...
def process_message_revoke(msg):
    ...
    if msg.we_type in ["Picture", "Recording"]:
        try:
            ...
            logger.warning("process_message_revoke: download %s to .Cache/", msg.msg_file)
        except Exception as excep:
            logger.error("process_message_revoke: download %s to .Cache/ error: %s", msg.msg_file, excep)
    ...
    if msg.we_type == "Note" and msg.we_text.find("撤回了一条消息") >= 0:
        ...
        if not old_msg:
            logger.warning("process_message_revoke: no message id in my.msg_store")
            return
...
@my.msg_register([TEXT, PICTURE, RECORDING, VIDEO, MAP, CARD, NOTE, SHARING, ATTACHMENT], isFriendChat=True)
def text_reply(msg):
    """
    ...
    if msg.we_type not in ["Text", "Picture", "Recording", "Card", "Note", "Sharing"]:
        logger.warning("process_message_group: message type isn't included, ignored")
        return

    if msg.we_type == "Text":
        from_user = msg.from_user_name
        recv_msg = msg.we_text
        back_msg = xrobot.response(recv_msg, from_user)
        my.send_msg(back_msg, from_user)
    # 处理撤回消息
    process_message_revoke(msg)
    return


@my.msg_register([TEXT, PICTURE, RECORDING, VIDEO, MAP, CARD, NOTE, SHARING, ATTACHMENT], isGroupChat=True)
def group_reply(msg):
    # 处理群消息
    msg = Message(msg)
